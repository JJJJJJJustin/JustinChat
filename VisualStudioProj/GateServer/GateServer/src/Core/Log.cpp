...
namespace JChat {
	...
	void Log::Init()
	{
		...
		logSinks.emplace_back(std::make_shared<spdlog::sinks::basic_file_sink_mt>("JustinChat.log", true));		// 指定一个日志目标，输出到 "Nut.log" ，并在每次启动时清空文件 (true 表示清空，false 表示追加写入)。
		...
		//创建日志记录器
		s_CoreLogger = std::make_shared<spdlog::logger>("JstChat", begin(logSinks), end(logSinks));			// Logger with range on sinks: logger(std::string name, It begin, It end) 传入所有需要使用的日志目标
